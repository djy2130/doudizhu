...
AI.formPokers=function(cards){
	...
	for(var i=0;i<cards.length;i++){
		...
		var pointTmp=-1;
		if(cards[i]==53){
			typeTmp=AIPoker.JOKERB;
			pointTmp=16;
		}
		else if(cards[i]==54){
			typeTmp=AIPoker.JOKERA;
			pointTmp=17;
		}
		else if((cards[i]-1)%4==0){
			typeTmp=AIPoker.FANGKUAI;
			pointTmp=Math.floor(cards[i]/4)+1+2;
		}
		else if((cards[i]-2)%4==0){
			typeTmp=AIPoker.MEIHUA;
			pointTmp=Math.floor(cards[i]/4)+1+2;
		}
		else if((cards[i]-3)%4==0){
			typeTmp=AIPoker.HONGTAO;
			pointTmp=Math.floor(cards[i]/4)+1+2;
		}
		else if(cards[i]%4==0){
			typeTmp=AIPoker.HEITAO;
			pointTmp=Math.floor(cards[i]/4)+2;
		}
		packs.push(new AIPoker(pointTmp, typeTmp));
//		cc.log("type:"+typeTmp.name+","+pointTmp+";");
	};
	return packs;
}

AI.formPlayer=function(cards){	
	var player = new AIPlayer();
	player.pokers=AI.formPokers(cards);
	return player;
}

/**
 * @param preCards: [] 上家出的牌，从大到小排序。 传null，代表玩家自由出牌
 * @param myCards： [] 自己剩余的牌，从大到小排序
 * @return [] 自己要出的牌，从大到小排序。 null,没有大过上家的牌。
 */
AI.findBestPokers = function(preCards,myCards) {
	var player=AI.formPlayer(myCards);
	var pokers=null;
	//自由出牌
	if(preCards==null){
		pokers=AI.freeFindPokers(player);
		if (pokers instanceof AIPoker) {
			pokers=[pokers];
		}
	}
	else{
		var compareTarget=AI.formPokers(preCards);
		pokers=AI.interFindBestPokers(player, AIRule.getType(compareTarget), compareTarget, true);
	}
	if(pokers==null){
		return null;
	}
	...
	for(var i=0;i<pokers.length;i++){
		var ourVar=-1;
		if(pokers[i].type==AIPoker.JOKERA){
			ourVar=54;
		}
		else if(pokers[i].type==AIPoker.JOKERB){
			ourVar=53;
		}
		else if(pokers[i].type==AIPoker.FANGKUAI){
			var begVar=((pokers[i].point-3)*4)+1;
			ourVar=begVar;
		}
		else if(pokers[i].type==AIPoker.MEIHUA){
			var begVar=((pokers[i].point-3)*4)+1;
			ourVar=begVar+1;
		}
		else if(pokers[i].type==AIPoker.HONGTAO){
			var begVar=((pokers[i].point-3)*4)+1;
			ourVar=begVar+2;
		}
		else if(pokers[i].type==AIPoker.HEITAO){
			var begVar=((pokers[i].point-3)*4)+1;
			ourVar=begVar+3;
		}
		sendCards[i]=ourVar;
...
AI.interFindBestPokers = function(player, type, compareTarget, includeBomb) {
	...
	for ( var i = result.length - 1; i >= 0; i--) {
		...
		if (group.length == type.numPokers) {
			return group;
		} else if (AIRule.compare(group, compareTarget)) {
...
AI.freeFindPokers = function(player) {
	...
	//第一优先考虑把所有牌都出去
	var type = AIRule.getType(pokers);
	if (type) {
		//如果有双王或炸弹，则忽略
	...
	if (len >= 5)
		p = this.findAllByType(player, GroupType.wulianfeijidaichibang);
	if (len >= 4 && !p.length)
		p = this.findAllByType(player, GroupType.silianfeijidaisidui);
	if (len >= 4 && !p.length)
		p = this.findAllByType(player, GroupType.silianfeijidaichibang);
	if (len >= 3 && !p.length)
		p = this.findAllByType(player, GroupType.sanlianfeijidaisandui);
	if (len >= 3 && !p.length)
		p = this.findAllByType(player, GroupType.sanlianfeijidaichibang);
	if (len >= 2 && !p.length)
		p = this.findAllByType(player, GroupType.feijidaierdui);
	if (len >= 2 && !p.length)
		p = this.findAllByType(player, GroupType.feijidaichibang);
	if (len >= 5 && !p.length)
		p = this.findAllByType(player, GroupType.wulianfeiji);
	if (len >= 4 && !p.length)
		p = this.findAllByType(player, GroupType.silianfeiji);
	if (len >= 3 && !p.length)
		p = this.findAllByType(player, GroupType.sanlianfeiji);
	if (len >= 2 && !p.length)
		p = this.findAllByType(player, GroupType.erlianfeiji);
	if (p.length) {
	...
	if (len >= 10)
		p = this.findAllByType(player, GroupType.shiliandui);
	if (len >= 9 && !p.length)
		p = this.findAllByType(player, GroupType.jiuliandui);
	if (len >= 8 && !p.length)
		p = this.findAllByType(player, GroupType.baliandui);
	if (len >= 7 && !p.length)
		p = this.findAllByType(player, GroupType.qiliandui);
	if (len >= 6 && !p.length)
		p = this.findAllByType(player, GroupType.liuliandui);
	if (len >= 5 && !p.length)
		p = this.findAllByType(player, GroupType.wuliandui);
	if (len >= 4 && !p.length)
		p = this.findAllByType(player, GroupType.siliandui);
	if (len >= 3 && !p.length)
		p = this.findAllByType(player, GroupType.sanliandui);
	if (p.length) {
		var group = p[p.length - 1];
		return group;
	}
	...
	//顺子
	p = this.findAllByType(player, GroupType.shierzhangshunzi);
	if (!p.length)
		p = this.findAllByType(player, GroupType.shiyizhangshunzi);
	if (!p.length)
		p = this.findAllByType(player, GroupType.shizhangshunzi);
	if (!p.length)
		p = this.findAllByType(player, GroupType.jiuzhangshunzi);
	if (!p.length)
		p = this.findAllByType(player, GroupType.bazhangshunzi);
	if (!p.length)
		p = this.findAllByType(player, GroupType.qizhangshunzi);
	if (!p.length)
		p = this.findAllByType(player, GroupType.liuzhangshunzi);
	if (!p.length)
		p = this.findAllByType(player, GroupType.wuzhangshunzi);
	if (p.length) {
		var group = p[p.length - 1];
		return group;
	}
...
AI.findAllByType = function(player, type, compareTarget, includeBomb) {
	...
	if (type == GroupType.huojian) {
		return player.twoking;
	} else if (type == GroupType.zhadan) {
		result = player.bomb;
	} else if (type == GroupType.danzhang) {
	...
	} else if (type == GroupType.duizi) {
		result = result.concat(player.pair);
		this._slicePokerFromList(player.triple, result, 2);
	} else if (type == GroupType.sanzhang) {
		result = result.concat(player.triple);
	} else {
		switch (type) {
		case GroupType.sandaiyi:
			result = this.findTarget(player, 3, 1, 1);
			break;
		case GroupType.sandaier:
			result = this.findTarget(player, 3, 1, 2);
			break;
		case GroupType.sidaier:
			result = this.findTarget(player, 4, 1, 2);
			break;
		case GroupType.sidaierdui:
			result = this.findTarget(player, 4, 1, 4);
			break;
		case GroupType.sanliandui:
		case GroupType.siliandui:
		case GroupType.wuliandui:
		case GroupType.liuliandui:
		case GroupType.qiliandui:
		case GroupType.baliandui:
		case GroupType.jiuliandui:
		case GroupType.shiliandui:
			result = this.findTarget(player, 2, type.numPokers / 2, 0);
			break;
		case GroupType.erlianfeiji:
		case GroupType.sanlianfeiji:
		case GroupType.silianfeiji:
		case GroupType.wulianfeiji:
		case GroupType.liulianfeiji:
			result = this.findTarget(player, 3, type.numPokers / 3, 0);
			break;
		case GroupType.feijidaichibang:
			result = this.findTarget(player, 3, 2, 2);
			break;
		case GroupType.feijidaierdui:
			result = this.findTarget(player, 3, 2, 4);
			break;
		case GroupType.sanlianfeijidaichibang:
			result = this.findTarget(player, 3, 3, 3);
			break;
		case GroupType.sanlianfeijidaisandui:
			result = this.findTarget(player, 3, 3, 6);
			break;
		case GroupType.silianfeijidaichibang:
			result = this.findTarget(player, 3, 4, 4);
			break;
		case GroupType.silianfeijidaisidui:
			result = this.findTarget(player, 3, 4, 8);
			break;
		case GroupType.wulianfeijidaichibang:
	...
	if (compareTarget) {
		for (i = 0; i < result.length; i++) {
			...
			if (!(group instanceof Array))
				group = [ group ];
			if (!AIRule.compare(group, compareTarget)) {
				result.splice(i, 1);
				i--;
			}
		}
